...
def init():
    global number_ComboBox, path, IP, ku_name, name, mima, starttime, endtime, num1, num2
    # 所需变量，大多不需要提前赋值
    number_ComboBox = 0  # 用于路径的下拉菜单
    # 设置默认开始时间，一天前
    if str(datetime.now())[8:10] in ['01']:
        if str(datetime.now())[5:7] in ['05', '07', '10', '12']:
            starttime = str(datetime.now())[0:5] + str(int(str(datetime.now())[5:7]) - 1) + '-30' + str(datetime.now())[
                                                                                                    10:19]
        elif str(datetime.now())[5:7] in ['02', '04', '06', '08', '09', '11']:
            starttime = str(datetime.now())[0:5] + str(int(str(datetime.now())[5:7]) - 1) + '-31' + str(datetime.now())[
                                                                                                    10:19]
        elif str(datetime.now())[5:7] in ['03']:
            if str(datetime.now())[0:4] % 4 == 0:
                starttime = str(datetime.now())[0:5] + str(int(str(datetime.now())[5:7]) - 1) + '-29' + str(
                    datetime.now())[10:19]
            else:
                starttime = str(datetime.now())[0:5] + str(int(str(datetime.now())[5:7]) - 1) + '-28' + str(
                    datetime.now())[10:19]
        else:
    ...
    if jilu:
        path = jilu[0][:-1]  # 路径
        IP = jilu[1][:-1]  # IP地址
        ku_name = jilu[2][:-1]  # 数据库名称
        name = jilu[3][:-1]  # 用户名
        mima = jilu[4][:-1]
    else:
        path = 'C:\\Users\\HP\\Desktop\\未命名.xlsx'  # 路径
        IP = 'raspberrypi'  # IP地址
        ku_name = "data1"  # 数据库名称
        name = "root"  # 用户名
        mima = "123"  # 密码
...
def save_shuju():
    ...
    while True:
        try:
            ...
            print(str(e))
            panel.logger.AppendText('数据库访问失败，正在重试，请稍等' + '\n')
            time.sleep(2)
    cursor = db.cursor()
    ...
    cursor.execute("select * from air_conditioning_adjustment_record WHERE  submission_date between"+' " '+starttime+' " '+" and "+' " '+endtime+' " '+";")
    dataFetched1 = cursor.fetchall()
    cursor.execute("select * from log WHERE  submission_date between"+' " '+starttime+' " '+" and "+' " '+endtime+' " '+";")
    dataFetched2 = cursor.fetchall()
    cursor.execute("select * from power_table WHERE  submission_date between"+' " '+starttime+' " '+" and "+' " '+endtime+' " '+";")
    dataFetched3 = cursor.fetchall()
    cursor.close()
    db.close()
    #print(dataFetched)
    ...
    data1 = []
    for row in dataFetched1:
        data1.append(list(row))
    data2 = []
    for row in dataFetched2:
        data2.append(list(row))
    data3 = []
    for row in dataFetched3:
        data3.append(list(row))


    #对前段和后段的选择进行操作，找到选取段的端点
    if num2 !=0:
        if num2<len(data):
            point1 = len(data) - num2
        else:
            point1 =0
        point2 = len(data)-1
    elif num1 !=0:
    ...
    #新建sheet
    shet1 = newwb.create_sheet(title="air_adjustment_record")
    shet2 = newwb.create_sheet(title="log")
    shet3 = newwb.create_sheet(title="power_table")
    ...
    for i in range(len(data1)):
        shet1.cell(i + 1, 1).value = data1[i]  # 单元格赋值
        shet1.cell(i + 1, 1).font = font  # 设置字体
    for i in range(len(data2)):
        shet2.cell(i + 1, 1).value = data2[i]  # 单元格赋值
        shet2.cell(i + 1, 1).font = font  # 设置字体
    for i in range(len(data3)):
        shet3.cell(i + 1, 1).value = data3[i]  # 单元格赋值
        shet3.cell(i + 1, 1).font = font  # 设置字体
    ...
    shet1.column_dimensions['A'].width = 45.0  # 每列列宽45
    shet2.column_dimensions['A'].width = 45.0  # 每列列宽45
    shet3.column_dimensions['A'].width = 45.0  # 每列列宽45

    #保存表格
    try:
        newwb.save(path)
        panel.logger.AppendText('数据保存成功' + '\n')
    except Exception as e:
        panel.logger.AppendText('此文件已打开，请关闭文件后重新下载' + '\n')
        print(str(e))
...
class ExamplePanel(wx.Panel):
    def __init__(self, parent):
        ...
        # 提示文字
        self.quote = wx.StaticText(self, label='控制器选项:', pos=(20, 10))
        self.quote = wx.StaticText(self, label='起始时间:', pos=(20, 110))
        self.quote = wx.StaticText(self, label='截止时间：', pos=(200, 110))
        self.quote = wx.StaticText(self, label='正数个数:', pos=(20, 170))
        self.quote = wx.StaticText(self, label='倒数个数：', pos=(200, 170))
        self.quote = wx.StaticText(self, label='路径:', pos=(20, 230))
        self.quote = wx.StaticText(self, label='下载前请先点击确认', pos=(145, 300))
        # 这个多行的文本框只是用来记录并显示events，#多行，只读文本框
        self.logger = wx.TextCtrl(self, pos=(395,20), size=(260,290),
                                  style=wx.TE_MULTILINE | wx.TE_READONLY)
        # 帮助按钮
        self.button4 = wx.Button(self, label='帮 助', pos=(20, 325))
        self.Bind(wx.EVT_BUTTON, self.Help, self.button4)
        # 确认按钮
        self.button1 = wx.Button(self, label='确 认', pos=(160, 325))
        self.Bind(wx.EVT_BUTTON, self.Config, self.button1)
        # 下载按钮
        self.button = wx.Button(self, label='下 载', pos=(300, 325))
        self.Bind(wx.EVT_BUTTON, self.Save, self.button)

        # 打开按钮
        self.button3 = wx.Button(self, label='打 开', pos=(440, 325))
        self.Bind(wx.EVT_BUTTON, self.OnOpen, self.button3)
        # 关闭按钮
        self.button2 = wx.Button(self, label='关 闭', pos=(580, 325))
        self.Bind(wx.EVT_BUTTON, self.OnExit, self.button2)


        # 路径下拉菜单
        self.sampleList = [path,'C:\\Users\\HP\\Desktop\\daik\\未命名.xlsx', 'C:\\Users\\HP\\Desktop\\未命名.xlsx','other']
        self.edithear = wx.ComboBox(self, pos=(20, 250), size=(330, -1),
        ...
        # IP地址下拉菜单
        self.sampleList1 = [IP,"raspberrypi",'192.168.1.101','192.168.1.102','192.168.1.103','192.168.1.104','192.168.1.105'
        ...
        # 数据库下拉菜单（预备增加数据库表格选项）
        self.sampleList2 = [ku_name,"data1","data2"]
        ...
        #数据库访问身份/密码
        self.editname = wx.TextCtrl(self, value=name ,pos=(20, 70), size=(150, -1))
        self.editmima = wx.TextCtrl(self, value=mima ,pos=(200, 70), size=(150, -1))

        # 数据时间区间
        self.begin = wx.TextCtrl(self, value=starttime,pos=(20, 130), size=(150, -1))
        self.end = wx.TextCtrl(self, value=endtime,pos=(200, 130), size=(150, -1))

        # 数据数量区间
        self.num_begin = wx.TextCtrl(self, value=str(num1), pos=(20, 190), size=(150, -1))
        self.num_end = wx.TextCtrl(self, value=str(num2), pos=(200, 190), size=(150, -1))
    ...
    def EvtComboBox(self, event):
        ...
        if path1 in ['other']:          #如果选择第三项则跳出路径选择框
            ...
            if dlg.ShowModal() == wx.ID_OK:         #如果确认就保存路径，有效下拉事件加1，用来在选择other项后文本框能够显示选择内容
                ...
                self.edithear.SetSelection(4 + number_ComboBox)
                path = path1
                number_ComboBox += 1
        else:
            path = path1
    ...
    def OnOpen(self, event):
        ...
        try:
            ...
            panel.logger.AppendText('未找到此文件，请确认是否下载' + '\n')
            print(str(e))
    ...
    def Help(self,event):
        helptext = "使用说明：\n控制器选项: 访问数据库的信息，需要时进行修改\n起始时间：所下载数据的开始时间，默认值为一天前此时刻\n" \
                   "截至时间：所下载数据的结束时间，空白时自动选择现在时刻\n正数个数：所选择时间段的前多少个，值为0时为不使用此功能\n" \
                   "倒数个数：所选择时间段的后多少个，值为0时为不使用此功能\n正数个数与倒数个数同时不为0时以倒数优先\n" \
                   "路径：所下载数据库信息的保存位置\n此程序为自动保存上次下载时的控制器选项和路径信息，保存在此程序同文件夹下的data.txt文档中\n" \
                   "下载前请先点击确认按钮，在右侧文本框中确认选择信息\n选择打开按钮即可打开刚下载的数据文件"
        help = wx.MessageDialog(self,helptext, caption='帮助',style=wx.OK,pos=wx.DefaultPosition)
        help.ShowModal()
...
#GUI开始运行
app = wx.App(False)
frame = wx.Frame(None, size=(690,400),title ='数据下载')
