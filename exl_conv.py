...
datas = []
with open("C:/Users/Dark/Desktop/dataA.txt") as fin:
    for line in fin:
        line = line[:-1]
...
#打开excel
workbook = openpyxl.load_workbook("C:/Users/Dark/Desktop/dataA.xlsx")
...
for i in newdata:
    ...
    #geshu = int(i[0][33:35])
    i[0] = i[0][:8]+' '+str(xuhao).zfill(2)+' '+str(wendu)+' '+str(shidu)+' '+str(dianya)+' '+str(jiange)+' '+i[0][30:]

#将数据分到对应子列表集
DATA = [[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
for x in newdata:
    y = x[0][9:11]
    DATA[int(y)-1].append(x)
...
#保存
workbook.save('C:/Users/Dark/Desktop/dataA.xlsx')
